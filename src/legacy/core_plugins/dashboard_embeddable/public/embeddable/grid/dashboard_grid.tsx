...
import { EmbeddableFactory, ViewMode } from 'plugins/embeddable_api/index';
// @ts-ignore
...
import { GridData, PanelState, PanelStateMap } from '../types';
...
interface Props extends ReactIntl.InjectedIntlProps {
  ...
  getEmbeddableFactory: (panelType: string) => EmbeddableFactory | undefined;
  dashboardViewMode: ViewMode.EDIT | ViewMode.VIEW;
...
class DashboardGridUi extends React.Component<Props, State> {
  ...
  public renderDOM() {
    ...
    return _.map(panelsInOrder, panel => {
      const expandPanel = maximizedPanelId !== undefined && maximizedPanelId === panel.embeddableId;
      const hidePanel = maximizedPanelId !== undefined && maximizedPanelId !== panel.embeddableId;
      const classes = classNames({
      ...
      return (
        ...
        >
          <DashboardPanel
            ...
            viewOnlyMode={this.props.dashboardViewMode === ViewMode.VIEW}
          />
        </div>
      );
    });
